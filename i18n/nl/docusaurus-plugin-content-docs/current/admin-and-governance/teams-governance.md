...
```mermaid
flowchart TD
    Start{"Bestaat er al een Team met hetzelfde doel en dezelfde doelgroep?"}
    Belongs{"Hoort het werk bij een bestaand Team?"}
    Standard{"Kan een standaardkanaal de samenwerking ondersteunen?"}
    Access{"Hoort het afgeschermde werk bij dit Team en volgt het dezelfde levenscyclus?"}
    Distinct{"Heeft het werk een eigen doelgroep, eigenaarschap of levenscyclus?"}
    Independent{"Moet de samenwerking zelfstandig worden beoordeeld en beëindigd?"}

    Reuse["Gebruik het bestaande Team of een kanaal daarin"]
    Channel["Gebruik een standaardkanaal"]
    Private["Overweeg een privékanaal"]
    NewTeam["Maak een afzonderlijk Team"]
    Review["Beoordeel de aanvraag met een key-user"]

    Start -->|Ja| Reuse
    Start -->|Nee| Belongs
    Belongs -->|Ja| Standard
    Belongs -->|Nee| Distinct
    Standard -->|Ja| Channel
    Standard -->|Nee| Access
    Access -->|Ja| Private
    Access -->|Nee| Distinct
    Distinct -->|Ja| NewTeam
    Distinct -->|Nee| Independent
    Independent -->|Ja| NewTeam
    Independent -->|Geen duidelijke uitkomst| Review
```
