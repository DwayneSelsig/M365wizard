...
Een SharePoint-bibliotheek bereikt 5.000 bestanden. Een weergave werkt niet meer of een flow mist records. De voor de hand liggende conclusie is dat de bibliotheek vol is.
...
Dit is een wat diepgaander SharePoint-onderwerp. De meeste gebruikers en veel bibliotheken krijgen er nooit mee te maken. Maar zodra een weergave, flow, migratie of wijziging van machtigingen tegen een grens aanloopt, vertelt de zichtbare foutmelding zelden het hele verhaal. Je moet eerst begrijpen welke technische route vastloopt en waarom.

Splits een bibliotheek dus niet alleen omdat deze meer dan 5.000 bestanden bevat. Vraag eerst hoe mensen en systemen de bestanden vinden, tonen, synchroniseren, beveiligen en verwerken. Een goede diagnose komt vóór de oplossing.
...
De praktische regel is eenvoudig: houd veelgebruikte weergaven en queries klein, maar niet per se de bibliotheek.
...
```mermaid
flowchart TD
    Library["Grote SharePoint-bibliotheek"]
    ...
    View --> ViewRule["Rechtstreekse bibliotheekbewerking<br/>List View Threshold is van toepassing<br/>Indexen en filters zijn belangrijk"]
    Search --> SearchRule["Afzonderlijke zoekindex<br/>Valt niet onder de LVT<br/>Actualiteit van de index is belangrijk"]
    Sync --> SyncRule["Synchronisatieclient<br/>5.000 is geen synchronisatiegrens<br/>Schaal van de client is belangrijk"]
    Automation --> AutoRule["Connectoraanvragen<br/>Filter en gebruik paginering<br/>Neem niet aan dat één aanvraag alles teruggeeft"]
```
...
De SharePoint-acties **Get items** en **Get files** geven standaard 100 items terug. Je kunt **Top Count** verhogen tot 5.000. Er is ook een minder duidelijke valkuil. In een lijst met meer dan 5.000 items kan **Get items** geen overeenkomsten teruggeven als de passende records buiten de eerste 5.000 items staan. Microsoft adviseert voor deze situatie **pagination** (paginering) in te schakelen.
...
**Ontwerp voor het toegangspatroon en niet alleen voor het aantal items.**
