...
```mermaid
flowchart TD
    Library["Large<br/>SharePoint library"]
    ...
    View --> ViewRule["Direct library<br/>operation<br/>List View Threshold<br/>applies<br/>Indexes and filters<br/>matter"]
    Search --> SearchRule["Separate search index<br/>Not governed by the LVT<br/>Index freshness matters"]
    Sync --> SyncRule["Sync client<br/>5,000 is not a<br/>sync ceiling<br/>Client scale matters"]
    Automation --> AutoRule["Connector requests<br/>Filter and paginate<br/>Do not assume one request<br/>returns all"]
```
